...
function gradientFromSlug(slug: string) {
  ...
  return `linear-gradient(160deg, hsl(${hue} 50% 22%), hsl(${(hue + 40) % 360} 50% 14%))`;
}
      ...
      <div className="columns-1 gap-5 sm:columns-2 lg:columns-3">
        {dishes.map((dish) => {
          ...
          const usePlaceholder =
            dish.slug === "placeholder-dish" || errored[key];

          return (
            <button
              key={key}
              onClick={() => setSelected(dish)}
              className="mb-5 w-full break-inside-avoid overflow-hidden rounded border border-[var(--border)] bg-[var(--surface)] text-left transition-colors hover:border-[var(--accent)]/30"
            >
              ...
              <div className="p-4">
                <h3 className="font-display text-xl italic">{dish.title}</h3>
